...
var waterCompositionMaterial : Material;// = null; private
...
function Start () 
{
	...
	for (L0D=0;L0D<max_L0D;L0D++) {
		tiles_L0D.Add (new Array());
 	}
	...
	for (y=0;y<tiles_y;y++) {
		for (x=0;x<tiles_x;x++) {
			...
			(tiles_L0D[chDist] as Array).Add ((tile.GetComponent(MeshFilter) as MeshFilter).mesh);
		}
	}
...
function GenerateHeightmap () {
	...
	for (L0D=0;L0D<max_L0D;L0D++) {
		verticesL0D = new Vector3[(height/Mathf.Pow(2,L0D)+1) * (width/Mathf.Pow(2,L0D)+1)];
		uvL0D = new Vector2[(height/Mathf.Pow(2,L0D)+1) * (width/Mathf.Pow(2,L0D)+1)];
		idx = 0;
 
		for (y=0;y<g_height;y+=Mathf.Pow(2,L0D)) {
			for (x=0;x<g_width;x+=Mathf.Pow(2,L0D)) {
				verticesL0D[idx] = vertices[g_width * y + x];
				uvL0D[idx++] = uv[g_width * y + x];
			}			
		}
		for (k=0;k<(tiles_L0D[L0D] as Array).length;k++) {
			meshL0D = (tiles_L0D[L0D] as Array)[k] as Mesh;
			meshL0D.vertices = verticesL0D;
			meshL0D.uv = uvL0D;
		}		
	}
	...
	for (L0D=0;L0D<max_L0D;L0D++) {
		index = 0;
		width_L0D = width / Mathf.Pow(2,L0D) + 1;
		triangles = new int[(height/Mathf.Pow(2,L0D) * width/Mathf.Pow(2,L0D)) * 6];
		for (y=0;y<height/Mathf.Pow(2,L0D);y++) {
			for (x=0;x<width/Mathf.Pow(2,L0D);x++) {
				// For each grid cell output two triangles
				triangles[index++] = (y     * width_L0D) + x;
				triangles[index++] = ((y+1) * width_L0D) + x;
				triangles[index++] = (y     * width_L0D) + x + 1;

				triangles[index++] = ((y+1) * width_L0D) + x;
				triangles[index++] = ((y+1) * width_L0D) + x + 1;
				triangles[index++] = (y     * width_L0D) + x + 1;
			}
		}
		for (k=0;k<(tiles_L0D[L0D] as Array).length;k++) {
			meshL0D = (tiles_L0D[L0D] as Array)[k] as Mesh;
			meshL0D.triangles = triangles;
		}
	}
...
//		uvs[i+offset] = uv;
	...
	if (!forceOriginalShader)
	{
		...
		//if (waterCompositionMaterial) { 
		waterCompositionMaterial.SetColor("_WaterColor", waterColor); 
		//} else { print("No such material as waterCompositionMaterial!"); }
	...
	for (L0D=0;L0D<max_L0D;L0D++) {
		var den = Mathf.Pow(2,L0D);
		var itemcount = (height/den+1) * (width/den+1);
		
		tangentsL0D = new Vector4[itemcount];
		verticesL0D = new Vector3[itemcount];
		normalsL0D = new Vector3[itemcount];
//		uvL0D = new Vector2[(height/Mathf.Pow(2,L0D)+1) * (width/Mathf.Pow(2,L0D)+1)];
		idx = 0;
		...
		for (y=0;y<g_height;y+=den) {
			for (x=0;x<g_width;x+=den) {
				idx2 = g_width * y + x;
				verticesL0D[idx] = vertices[idx2];
//				uvL0D[idx] = uvs[g_width * y + x];
				tangentsL0D[idx] = tangents[idx2];
				normalsL0D[idx++] = normals[idx2];
			}			
		}
		for (k=0;k< (tiles_L0D[L0D] as Array).length ;k++) {
			meshL0D = (tiles_L0D[L0D] as Array)[k] as Mesh;
			meshL0D.vertices = verticesL0D;
			meshL0D.normals = normalsL0D;
//			meshL0D.uv = uvL0D;
			meshL0D.tangents = tangentsL0D;
		}		
	}
}
