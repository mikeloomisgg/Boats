...
public class BuildAtlases : ScriptableWizard
{
	...
	[MenuItem("Tools/A&B Software/Build Atlases &a")]
	...
	[MenuItem("Tools/A&B Software/Build Atlas for Material &m")]
	...
	// Validator:
	[MenuItem("Tools/A&B Software/Build Atlas for Material &m", true)]
	...
	// Context menu:
	[MenuItem("CONTEXT/Material/Build Atlas")]
	...
	// Validator:
	[MenuItem("CONTEXT/Material/Build Atlas", true)]
			...
			for (int j = 0; j < spr.SourceTextures.Length; ++j)
			{
				if (spr.SourceTextures[j] != null)
				{
					...
					if (-1 >= texList.textures.IndexOf(spr.SourceTextures[j]))
					{
						...
						ProcessFrame(spr.SourceTextures[j], ref newTex, ref frameInfo);
				...
				else
				{
					...
					Debug.LogWarning("Null texture reference detected on sprite \"" + ((SpriteRoot)spr).name + "\"! Please verify that this was intentional.");
#endif
				}
			}
		}
...
// 		atlas = (Texture2D) AssetDatabase.LoadAssetAtPath(projectRelativePath, typeof(Texture2D));
	...
	void ProcessFrame(Texture2D tex, ref Texture2D newTex, ref SPRITE_FRAME frameInfo)
	{
